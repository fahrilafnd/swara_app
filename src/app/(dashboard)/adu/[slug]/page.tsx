import Link from "next/link";

export default function AduSession() {
    return (
        <div className="font-lexend h-full flex flex-col pt-8 pb-5">
            <div className="bg-[#F8C56A] flex w-max mx-auto pl-4 rounded-2xl overflow-hidden mb-8">
                <p className="text-[#EF5C00] px-4 my-auto underline">Topik Pembicaraan</p>
                <p className="bg-white text-[#F07122] py-5 px-5 rounded-2xl">Merancang Masa Depan : Membangun Karier di Era Digital</p>
            </div>
            <div className="bg-white p-6 rounded-t-2xl flex justify-between items-end">
                {/* kamera kiri */}
                <div>
                    <img className="w-[600px] rounded-2xl" src="https://www.totaljobs.com/advice/wp-content/uploads/how-to-set-up-and-test-cammio-for-your-video-interview.jpg" alt="" />
                </div>
                {/* kamera kanan */}
                <div>
                    <img className="w-[700px] rounded-2xl border-8 border-[#F07122]" src="https://media.gettyimages.com/id/1635403579/video/video-call-communication-face-or-man-wave-hello-discussion-and-consulting-with-webinar.jpg?s=640x640&k=20&c=ok5TDCZBzxcWIE5ZSwZgSkdWRl29uj1aiXvLpN7GMkw=" alt="" />
                </div>
            </div>
            <div className="flex justify-between">
                <div className="flex items-end h-max">
                    <div className="bg-white flex items-center py-3 px-5 rounded-2xl rounded-tl-none">
                        <img className="w-[50px] h-[50px] object-cover object-center rounded-full mr-4" src="https://i.pinimg.com/736x/5b/03/a2/5b03a2f8bd8d357c97754d572a3b816b.jpg" alt="" />
                        <p className="text-lg font-bold text-[#39363D]">Kimberly</p>
                    </div>
                </div>
                <div className="flex items-end h-max">
                    <span className="bg-[#F07122] text-white text-xl font-medium mr-4 px-3 py-2 rounded-2xl">Your Turn!</span>
                    <div className="bg-white flex items-center py-3 px-5 rounded-2xl rounded-tr-none">
                        <p className="text-lg font-bold text-[#39363D]">Kimberly</p>
                        <img className="w-[50px] h-[50px] object-cover object-center rounded-full ml-4" src="https://i.pinimg.com/736x/5b/03/a2/5b03a2f8bd8d357c97754d572a3b816b.jpg" alt="" />
                    </div>
                </div>
            </div>
            <span className="w-max mx-auto text-[#F07122] bg-white font-semibold text-2xl py-2 px-4 rounded-2xl mb-12">02:11</span>
            <Link href={"/adu/result"} className="bg-[#F8C56A] text-lg text-white flex w-max items-center py-3 px-4 rounded-2xl border-2 border-[#F07122] mx-auto">
                <div className="mr-3">
                    <svg width="20" height="24" viewBox="0 0 20 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fillRule="evenodd" clipRule="evenodd" d="M0.84375 7.875C1.06753 7.875 1.28214 7.9639 1.44037 8.12213C1.59861 8.28036 1.6875 8.49497 1.6875 8.71875V9.84375C1.6875 10.9148 1.89847 11.9755 2.30836 12.965C2.71825 13.9546 3.31903 14.8537 4.07641 15.6111C4.83379 16.3685 5.73293 16.9693 6.72249 17.3791C7.71205 17.789 8.77266 18 9.84375 18C10.9148 18 11.9754 17.789 12.965 17.3791C13.9546 16.9693 14.8537 16.3685 15.6111 15.6111C16.3685 14.8537 16.9692 13.9546 17.3791 12.965C17.789 11.9755 18 10.9148 18 9.84375V8.71875C18 8.49497 18.0889 8.28036 18.2471 8.12213C18.4054 7.9639 18.62 7.875 18.8437 7.875C19.0675 7.875 19.2821 7.9639 19.4404 8.12213C19.5986 8.28036 19.6875 8.49497 19.6875 8.71875V9.84375C19.6876 12.3085 18.763 14.6835 17.0966 16.4995C15.4302 18.3155 13.1431 19.4402 10.6875 19.6515V22.2188C10.6875 22.4425 10.5986 22.6571 10.4404 22.8154C10.2821 22.9736 10.0675 23.0625 9.84375 23.0625C9.61997 23.0625 9.40536 22.9736 9.24713 22.8154C9.08889 22.6571 9 22.4425 9 22.2188V19.6515C6.54437 19.4402 4.25733 18.3155 2.59091 16.4995C0.924491 14.6835 -5.69285e-05 12.3085 2.62909e-09 9.84375V8.71875C2.62909e-09 8.49497 0.0888947 8.28036 0.247129 8.12213C0.405362 7.9639 0.619974 7.875 0.84375 7.875Z" fill="white"/>
                    <path fillRule="evenodd" clipRule="evenodd" d="M9.84375 0C8.12813 0 6.48278 0.681527 5.26965 1.89465C4.05653 3.10778 3.375 4.75313 3.375 6.46875V9.84375C3.375 11.5594 4.05653 13.2047 5.26965 14.4178C6.48278 15.631 8.12813 16.3125 9.84375 16.3125C11.5594 16.3125 13.2047 15.631 14.4178 14.4178C15.631 13.2047 16.3125 11.5594 16.3125 9.84375V6.46875C16.3125 4.75313 15.631 3.10778 14.4178 1.89465C13.2047 0.681527 11.5594 0 9.84375 0ZM12.0938 10.6875C12.3175 10.6875 12.5321 10.5986 12.6904 10.4404C12.8486 10.2821 12.9375 10.0675 12.9375 9.84375C12.9375 9.61997 12.8486 9.40536 12.6904 9.24713C12.5321 9.08889 12.3175 9 12.0938 9H7.59375C7.36997 9 7.15536 9.08889 6.99713 9.24713C6.8389 9.40536 6.75 9.61997 6.75 9.84375C6.75 10.0675 6.8389 10.2821 6.99713 10.4404C7.15536 10.5986 7.36997 10.6875 7.59375 10.6875H12.0938ZM11.8125 6.46875C11.8125 6.69253 11.7236 6.90714 11.5654 7.06537C11.4071 7.2236 11.1925 7.3125 10.9688 7.3125H8.71875C8.49497 7.3125 8.28036 7.2236 8.12213 7.06537C7.9639 6.90714 7.875 6.69253 7.875 6.46875C7.875 6.24497 7.9639 6.03036 8.12213 5.87213C8.28036 5.71389 8.49497 5.625 8.71875 5.625H10.9688C11.1925 5.625 11.4071 5.71389 11.5654 5.87213C11.7236 6.03036 11.8125 6.24497 11.8125 6.46875Z" fill="white"/>
                    </svg>
                </div>
                Mulai Berbicara
            </Link>
            {/* <button className="bg-[#FF5A7E] text-lg text-white flex w-max items-center py-3 px-4 rounded-2xl border-2 border-[#F07122] mx-auto">
                <div className="mr-3">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M11.9998 13.3998L7.0998 18.2998C6.91647 18.4831 6.68314 18.5748 6.3998 18.5748C6.11647 18.5748 5.88314 18.4831 5.6998 18.2998C5.51647 18.1165 5.4248 17.8831 5.4248 17.5998C5.4248 17.3165 5.51647 17.0831 5.6998 16.8998L10.5998 11.9998L5.6998 7.0998C5.51647 6.91647 5.4248 6.68314 5.4248 6.3998C5.4248 6.11647 5.51647 5.88314 5.6998 5.6998C5.88314 5.51647 6.11647 5.4248 6.3998 5.4248C6.68314 5.4248 6.91647 5.51647 7.0998 5.6998L11.9998 10.5998L16.8998 5.6998C17.0831 5.51647 17.3165 5.4248 17.5998 5.4248C17.8831 5.4248 18.1165 5.51647 18.2998 5.6998C18.4831 5.88314 18.5748 6.11647 18.5748 6.3998C18.5748 6.68314 18.4831 6.91647 18.2998 7.0998L13.3998 11.9998L18.2998 16.8998C18.4831 17.0831 18.5748 17.3165 18.5748 17.5998C18.5748 17.8831 18.4831 18.1165 18.2998 18.2998C18.1165 18.4831 17.8831 18.5748 17.5998 18.5748C17.3165 18.5748 17.0831 18.4831 16.8998 18.2998L11.9998 13.3998Z" fill="white"/>
                    </svg>
                </div>
                Cukup
            </button> */}
        </div>
    )
}